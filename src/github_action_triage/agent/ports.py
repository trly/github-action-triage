...
class FailureSummary(BaseModel):
    ...
    logs_snippet: str = Field(..., description="Relevant logs or error messages from the failure")
...
class WorkflowRunFailureEvent(BaseModel):
    ...
    repository: RepositoryRef = Field(..., description="Repository where the workflow ran")
    workflow: WorkflowRef = Field(..., description="Workflow run metadata")
    failure: FailureSummary = Field(..., description="Failure details and diagnostics")
...
class FailureContext(BaseModel):
    ...
    logs_url: str | None = Field(
        default=None, description="URL to download job logs (if available)"
    )
...
class RemediationProposal(BaseModel):
    issue_title: str = Field(..., description="Short, concise issue title (< 80 characters)")
